...
// This is a prerendered SPA: the HTML only exists after `npm run build:prerender`,
// so the audit reads dist/, not the source tree.
const LANGS = [
...
console.log(`  Rules that bind those paragraphs: Malaysian Food Regulations 1985 Reg 18(6) —`);
console.log(`  nothing may be worded as preventing, alleviating, treating or curing a disease.`);
console.log(`  Any price, address, phone number or opening hour must come from`);
console.log(`  src/data/storeFacts.json, never be typed into the copy.\n`);
